...
- 每条评价使用 `review-feature`，左侧 52px 圆形头像，右侧是客户名、短标题和当前语言的评价译文。
- 客户名使用 `review-name`，标题用简短一句话概括评价重点，不写夸张营销语。
- `review-copy` 应保留客户真实意思，但可以压缩成适合网页阅读的自然译文；英文、法语、德语页面显示对应语言译文，中文页面显示中文译文。
- 2026-06-18 起，Google 评价模块不再输出“查看原文 / Read original”折叠区。真实原文只作为内部核验依据，不展示在正式页面里，避免页面变重和评价区出现空白折叠块。
